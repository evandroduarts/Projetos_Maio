...
tentativasRestantesElemento.textContent = `Você tem ${tentativas} tentativas.`;
...
function verificarTentativa() {
    ...
    tentativas--;
    tentativasRestantesElemento.textContent = `Você tem ${tentativas} tentativas restantes.`;

    if (tentativaUsuario === numeroSecreto) {
        mensagemElemento.textContent = `🎉 Parabéns! Você adivinhou o número ${numeroSecreto} em ${10 - tentativas} tentativas.`;
        desabilitarInput();
    } else if (tentativaUsuario < numeroSecreto) {
        mensagemElemento.textContent = '🔽 Muito baixo! Tente um número maior.';
    } else {
        mensagemElemento.textContent = '🔼 Muito alto! Tente um número menor.';
    }

    if (tentativas === 0 && tentativaUsuario !== numeroSecreto) {
        mensagemElemento.textContent = `❌ Você perdeu! O número era ${numeroSecreto}.`;
        desabilitarInput();
    }
